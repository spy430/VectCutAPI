...
调用 `save_draft` 会在`capcut_server.py`当前目录下生成一个 `dfd_` 开头的文件夹，将其**整个文件夹**复制到剪映/CapCut 草稿目录，即可在应用中看到生成的草稿。

### 4. 常见问题（含嵌入 Electron / 内置到应用）

**Q：导出的草稿在剪映里打开是空的？**  
A：已修复。原因是 `draft_info.json` 里素材的 `path` 必须为**相对草稿目录**的路径（如 `assets/video/xxx.mp4`），否则剪映找不到文件。当前版本在保存前会自动写入相对路径。请确保：① 复制到剪映的是「整个 `dfd_xxx` 文件夹」（内含 `draft_info.json` 和 `assets` 子目录）；② 使用最新代码。

**Q：提示 9001 端口被占用？**  
A：多出现在把本服务内置到 Electron 等应用时：每次导出都新开了一个 Python 进程，旧进程未退出导致端口占用。处理方式任选其一：  
- **推荐**：应用内只启动**一个** VectCutAPI 服务进程，多次导出复用同一进程（只调 API，不重复起服务）。  
- 每次导出前先结束上一次启动的 `python capcut_server.py` 进程再启动新进程。  
- 使用当前版本：若配置的端口被占用，服务会自动尝试下一个端口（如 9002、9003），并在控制台打印实际端口；Electron 侧若需固定端口，可在启动前检测端口是否空闲再决定是否起新进程。

**嵌入 Electron 时**：在 config.json 中可设置 `port`（默认 9000，示例里多为 9001）。前端请求请使用实际监听的端口；若使用端口自动递增，需从启动日志或健康检查接口获取当前端口。
